...
pub type TMQProducerKafka = FutureProducer; // Currently using Kafka as the MQ
pub type TMQProducerLockKafka = Arc<Mutex<TMQProducerKafka>>;
pub type TMQConsumerKafka = StreamConsumer; // Currently using Kafka as the MQ
pub type TMQConsumerLockKafka = Arc<Mutex<TMQConsumerKafka>>;

pub(crate) fn open_mq_connections_kafka(
    hostas_name_or_address: &HostType,
    host_port: &u16,
) -> (TMQProducerLockKafka, TMQConsumerLockKafka) {
    // define lamba to return ClientConfig so that I can deal with late bindings
    //let get_kafka_client_config = || {
    //    ClientConfig::new()
    //        .set(
    //            "bootstrap.servers",
    //            format!("{}:{}", host_address, host_port),
    //        )
    //        .set("group.id", "auth")
    ...
    let stream_consumer: StreamConsumer =
        //rdkafka::consumer::StreamConsumer::from_config(get_kafka_client_config())
        rdkafka::consumer::StreamConsumer::from_config(
        ClientConfig::new()
            .set(
                "bootstrap.servers",
                format!("{}:{}", hostas_name_or_address, host_port),
            )
            .set("group.id", "auth")
            .set("enable.auto.commit", "true")
            .set("auto.commit.interval.ms", "1000")
    ...
    stream_consumer
        ...
        .expect(format!("Failed to subscribe to Kafka topic '{}'", topic.clone()).as_str());
    ...
    //let future_producer = rdkafka::producer::FutureProducer::from_config(get_kafka_client_config())
    let future_producer = rdkafka::producer::FutureProducer::from_config(
        ClientConfig::new()
            .set(
                "bootstrap.servers",
                format!("{}:{}", hostas_name_or_address, host_port),
    ...
    (
        Arc::new(Mutex::new(future_producer)),
        Arc::new(Mutex::new(stream_consumer)),
    )
}

pub(crate) fn get_token_kafka<TFn>(
    mq_lock: &TMQProducerLockKafka,
    lhs_state_token: &str,
    fn_equ_op: TFn,
) -> Option<TokenData>
where
...
pub(crate) async fn post_new_login_kafka(
    db_connection: &TMQProducerLockKafka,
